...
def minimum(nums, min, i, result):
    """
    Ця функція призначена для зміни типу всіх елементів списку з str на int

    Args:
        nums:   список, в якому знаходиться найменший елемент
        min:    список, в якому елементи розташовані в порядку спадання
        i:      число, що збільшується на 1 з кожним повторенням функції
        result: число, мінімальний елемент списку
...
print("Мінімальний елемент списку = " + str(minimum(strlist, strlist[0],
                                                    0, [])))
